...
class AccountantRouter extends Actor with ActorLogging with GetOrCreateChild {
  ...
  def receive = LoggingReceive {
    ...
    case Accountant.GetOrders(account) =>
      getOrCreateChild(account).tell(Accountant.GetOrders(account), sender())

    case Accountant.CancelOrder(account, id) =>
      getOrCreateChild(account).tell(Accountant.CancelOrder(account, id), sender())
